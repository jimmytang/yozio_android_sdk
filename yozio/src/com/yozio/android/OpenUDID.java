...
public class OpenUDID {
  ...
  public final static String LOGTAG = "OpenUDID";
  ...
  // false if you don't wanna include READ_PHONE_STATE permission
  // we recommend adding BT permission over READ_PHONE_STATE permission as the
  // will be less privacy concerns
  private final static boolean _UseImeiFailback = false;
  // false if you don't wanna include BT permission or android 1.6
  ...
  public static void syncContext(Context mContext) {
    if (_openUdid == null) {
      ...
      try {
        openContext = mContext.createPackageContext("net.openudid.android",
                Context.CONTEXT_IGNORE_SECURITY);
        mContext = openContext;
      } catch (NameNotFoundException e1) {
      }

      SharedPreferences mPreferences = mContext.getSharedPreferences(PREFS_NAME,
  ...
  private static void generateOpenUDIDInContext(Context mContext) {
    ...
    String _androidId = Secure.getString(mContext.getContentResolver(), Secure.ANDROID_ID);
    if (_androidId != null) {
  ...
  private static void generateSystemId() {
    ...
    String fp = String.format("%s/%s/%s/%s:%s/%s/%s:%s/%s/%d-%s-%s-%s-%s", Build.BRAND,
            Build.PRODUCT, Build.DEVICE, Build.BOARD, Build.VERSION.RELEASE, Build.ID,
            Build.VERSION.INCREMENTAL, Build.TYPE, Build.TAGS, Build.TIME, Build.DISPLAY,
            Build.HOST, Build.MANUFACTURER, Build.MODEL);
